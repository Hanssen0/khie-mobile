...
export function KhieScreen({ state, pairing, approval, queuedApprovalCount, network, signer, onScan, onPair, onCancelPairing, onConnectRelay, onUnpair, onRespond }: { state: KhieProviderSessionState; pairing: boolean; approval?: ApprovalItem; queuedApprovalCount: number; network: Network; signer?: Signer; onScan: () => void; onPair: (endpoint: string) => Promise<boolean>; onCancelPairing: () => void; onConnectRelay: (address: string) => Promise<boolean>; onUnpair: () => Promise<void>; onRespond: (approved: boolean) => void }) {
  ...
  const scrollToApproval = useCallback((event: LayoutChangeEvent) => { if (!approval) return; approvalOffsetY.current = event.nativeEvent.layout.y; if (approvalScrolledId.current === approval.id) return; approvalScrolledId.current = approval.id; requestAnimationFrame(() => scrollRef.current?.scrollTo({ y: khieContentOffsetY.current + approvalOffsetY.current, animated: true })); }, [approval]);
  const scrollToApprovalAfterSummary = useCallback(() => { if (!approval) return; requestAnimationFrame(() => requestAnimationFrame(() => scrollRef.current?.scrollTo({ y: khieContentOffsetY.current + approvalOffsetY.current, animated: true }))); }, [approval]);

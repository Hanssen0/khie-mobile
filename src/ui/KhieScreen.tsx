...
const APPROVAL_ACTION_DELAY_MS = 1_000;

export function KhieScreen({ state, pairing, approval, queuedApprovalCount, network, signer, onScan, onPair, onCancelPairing, onConnectRelay, onUnpair, onRespond }: { state: KhieProviderSessionState; pairing: boolean; approval?: ApprovalItem; queuedApprovalCount: number; network: Network; signer?: Signer; onScan: () => void; onPair: (endpoint: string) => Promise<boolean>; onCancelPairing: () => void; onConnectRelay: (address: string) => Promise<boolean>; onUnpair: () => Promise<void>; onRespond: (approved: boolean) => void }) {
  ...
  return <KeyboardAwareScrollView ref={scrollRef} bottomOffset={16} contentContainerStyle={styles.page} keyboardShouldPersistTaps="handled">
    ...
    {pairing ? (
      ...
      <Divider /><ApprovalPanel item={approval} queuedCount={queuedApprovalCount} network={network} signer={signer} onRespond={onRespond} onLayout={scrollToApproval} />
...
function ApprovalPanel({ item, queuedCount, network, signer, onRespond, onLayout }: { item?: ApprovalItem; queuedCount: number; network: Network; signer?: Signer; onRespond: (approved: boolean) => void; onLayout?: (event: LayoutChangeEvent) => void }) {
  const { t } = useI18n(); const theme = useTheme();

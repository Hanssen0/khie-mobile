...
export function SettingsScreen({ backend, network, profile, wallets, rpcUrls, themePreference, updateSettings, checkingForUpdates, currentVersion, buildCommit, appArchitecture, updateAvailable, updateAsset, biometricAvailable, biometricUnlock, masterPasswordSet, onChangeNetwork, onChangeBiometricUnlock, onChangeMasterPassword, onSelectWallet, onAddWallet, onManageTrustDevice, onRemoveWallet, onSaveRpcUrls, onChangeThemePreference, onChangeAutomaticUpdateChecks, onCheckForUpdates, onDownloadUpdate }: {
  ...
  onChangeNetwork: (network: Network) => void; onChangeBiometricUnlock: (enabled: boolean) => Promise<void>; onChangeMasterPassword: (oldPassword: string, newPassword: string) => Promise<void>; onSelectWallet: (walletId: string) => Promise<void>; onAddWallet: () => void; onManageTrustDevice: () => void; onRemoveWallet: (walletId: string) => Promise<void>; onSaveRpcUrls: (urls: NetworkRpcUrls) => Promise<void>; onChangeThemePreference: (preference: ThemePreference) => Promise<void>; onChangeAutomaticUpdateChecks: (enabled: boolean) => Promise<void>; onCheckForUpdates: () => Promise<void> | void; onDownloadUpdate: () => Promise<void>;
}) {
  const { t } = useI18n();
  ...
  return <>
    <KeyboardAwareScrollView bottomOffset={16} contentContainerStyle={[styles.page, styles.settingsPage]} keyboardShouldPersistTaps="handled">
      ...
      <PaperCard mode="elevated"><PaperCard.Title title={t("wallets")} leftStyle={styles.cardTitleLeft} left={(props) => <Icon {...props} source="wallet" />} />
        ...
        <PaperCard.Actions style={styles.cardActions}>{profile.kind === "cryptape-trust" ? <PaperButton mode="contained-tonal" icon="tune-variant" onPress={onManageTrustDevice}>{t("manageTrustDevice")}</PaperButton> : null}<PaperButton mode="contained" icon="plus" onPress={onAddWallet}>{t("addWallet")}</PaperButton></PaperCard.Actions>
      </PaperCard>
